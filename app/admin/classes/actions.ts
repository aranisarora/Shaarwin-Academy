 ...
 * upcoming session moves with it (same week, new slot) and booked clients
 * are told. Coaches who no longer fit are cleared for the engine to refill.
 */
export async function updateGroupClass(input: ClassUpdate): Promise<Result> {
  ...
  // Move upcoming sessions to the new slot where needed.
  ...
  for (const s of sessions ?? []) {
    ...
    if (newStart <= new Date()) continue; // don't move a session into the past
    ...
    if (moveErr) {
      // Usually the coach clashes at the new time — free the slot and let the engine refill.
  ...
  // Tell everyone who's booked onto a moved session (or if the venue changed).
...
/** Hard delete — only for classes nobody ever booked (e.g. created by mistake). */
  ...
  if (ids.length) {
    ...
    if ((count ?? 0) > 0) {
      return {
        ok: false,
        error:
          "People have booked this class, so it can't be deleted. Use “End class” instead — history stays safe.",
      };
    }
  }
