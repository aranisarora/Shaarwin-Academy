...
AS $function$
  select
    ...
    -- Nothing ever moves it for us: sweep_session_status used to auto-attend
    -- anything still confirmed 48h on, and that half was dropped in 0065 rather
    -- than keep inventing a register nobody kept. So an unmarked booking stays
    -- unmarked for good, and this figure is the truth on the evening and after.
    (select count(*)::int from bookings b
      where b.session_id = s.id
