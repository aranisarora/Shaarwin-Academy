...
// ── Offset-bearing timestamps (bluetick's diary) ─────────────────────────────
// The public timetable comes from bluetick, which returns every timestamp with
// the workspace's own UTC offset already attached ("2026-09-14T18:00:00+05:30").
// That string IS the academy wall clock, so these helpers read it directly
// rather than turning it into an instant and back through a timezone: an event
// at 00:30 must stay on the day bluetick filed it under, whatever the reader's
// clock says.

/** "2026-09-14T18:00:00+05:30" → "2026-09-14". The local day, as filed. */
export function isoWallDate(iso: string): string {
  return iso.slice(0, 10);
}

/** "2026-09-14T18:00:00+05:30" → "6:00 pm". No timezone maths. */
export function formatIsoWallClock(iso: string): string {
...
const WALL_DAY_LONG = new Intl.DateTimeFormat("en-GB", {
  timeZone: "UTC",
  weekday: "long",
  day: "numeric",
  month: "long",
...
/** "Monday 14 September" for a bare "YYYY-MM-DD" wall date. UTC-formatted for
 * the same reason as `formatWallDay`: the date carries no time to shift. */
export function formatWallDayLong(dateStr: string): string {
  const [y, m, d] = dateStr.split("-").map(Number);
  return WALL_DAY_LONG.format(new Date(Date.UTC(y, m - 1, d)));
}
